...
```bash
# Run the backend API (requires POSTGRES_URL env var)
make run
...
# Run all checks (typecheck, prettier, lint, vitest, build)
pnpm test
...
Visit http://localhost:5173 for frontend (proxies API calls to backend on :4000)
